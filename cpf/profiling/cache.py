...
$ python -m cpf.profiling.cache CDP2.properties /imaging/analysis/2008_12_04_Imaging_CDRP_for_MLPCN/CDP2/cache "Image_Metadata_ASSAY_WELL_ROLE = 'mock'"

Example usage as module:

>>> import cpf
>>> from cpf.profiling.cache import Cache
>>> from cpf.profiling.normalization import RobustLinearNormalization
>>> cpf.properties.LoadFile('CDP2.properties')
>>> cache = Cache('/imaging/analysis/2008_12_04_Imaging_CDRP_for_MLPCN/CDP2/cache')
>>> cc_mapping, cc_colnames = cpf.db.group_map('CompoundConcentration', reverse=True)
...
class Cache(object):
    ...
    def _plate_map(self):
        if self._cached_plate_map is None:
            self._cached_plate_map = cpf.util.unpickle1(self._plate_map_filename)
    ...
    def _create_cache_colnames(self, resume):
        ...
        if resume and os.path.exists(self._colnames_filename):
            return
        cols = cpf.db.GetColnamesForClassifier()
    ...
    def _create_cache_plate_map(self, resume):
        ...
        self._cached_plate_map = dict((tuple(row[1:]), row[0])
                                      for row in cpf.db.execute('select distinct %s, %s from %s'%
                                                                (cpf.properties.plate_id, 
                                                                 ', '.join(cpf.dbconnect.image_key_columns()),
                                                                 cpf.properties.image_table)))
        cpf.util.pickle(self._plate_map_filename, self._cached_plate_map)
    ...
    def _create_cache_image(self, plate, image_key, resume=False):
        ...
        if resume and os.path.exists(filename):
            return
        features = cpf.db.execute("""select %s from %s where %s""" % (
    ...
    def _create_cache_counts(self, resume):
        ...
        if resume and os.path.exists(self._counts_filename):
            return
        result = cpf.db.execute("""select {0}, count(*) from {1} group by {0}""".format(
                cpf.dbconnect.UniqueImageClause(), 
                cpf.properties.object_table))
        counts = np.array(result, dtype='i4')
        with cpf.util.replace_atomically(self._counts_filename) as f:
            np.save(f, counts)
