...
import cpf.util
from cpf.profiling import cache
...
class RobustLinearNormalizationTestCase(unittest.TestCase):
    ...
    def test_create_cache_percentiles_1_nofeatures(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.npy') as file:
            with patch('cpf.profiling.cache.logger') as logger:
                logger.warning = Mock()
    ...
    @patch('cpf.profiling.cache.make_progress_bar')
    @patch('cpf.profiling.cache._check_directory')
...
class CacheTestCase(unittest.TestCase):
    ...
    def test_plate_map(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.pickle') as file:
            cpf.util.pickle(file.name, original)
    ...
    @patch('cpf.util.pickle')
    @patch('cpf.db.execute')
    @patch('cpf.dbconnect.image_key_columns')
