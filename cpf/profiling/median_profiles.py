#!/usr/bin/env python
#
# python -m cpf.profiling.median_profiles -o treatment_profiles_mean.txt ~/src/az/properties/supplement.properties well_profiles_mean.txt Well CompoundConcentration
...
def aggregate_profiles(profiles, group_name, aggregator):
    ...
    input_group_r, input_colnames = cpf.db.group_map(profiles.group_name, reverse=True)
    input_group_r = dict((tuple(map(str, k)), v) 
                         for k, v in input_group_r.items())
    output_group, output_colnames = cpf.db.group_map(group_name)
