...
def organize_image_keys_per_group(image_keys, group):
    group_mapping, colnames = cpf.db.group_map(group)
    per_group = {}
    for image_key in image_keys:
...
class Subsample(object):
    ...
    def _compute(self, sample_size, filter, group, parallel, show_progress, 
        ...
        cache = Cache(self.cache_dir)
        if filter is None:
            image_keys = cpf.db.GetAllImageKeys()
        else:
            image_keys = cpf.db.GetFilteredImages(filter)
...
if __name__ == '__main__':
    ...
    # when unpickling.
    import cpf.profiling.subsample
    subsample = cpf.profiling.subsample.Subsample(
